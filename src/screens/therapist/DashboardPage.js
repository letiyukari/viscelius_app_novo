...
const TherapistDashboardPage = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setNotification({ message: 'Erro ao carregar lista de pacientes.', type: 'error' });
      setLoading(false);
    });
    return () => unsubscribe();
  }, [therapistUid]);
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      <div style={styles.searchFilterContainer}>
        <input type="text" style={styles.searchBar} placeholder="Buscar paciente por nome ou e-mail..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
        ...
        <div style={styles.patientGrid}>
          {filteredPatients.length > 0 ? (
            filteredPatients.map(patient => {
                const isNameHovered = hoveredNameId === patient.id;
                return (
                  <div key={patient.id} style={styles.patientCard}>
                    <div style={styles.cardBody}>
                      <div style={styles.cardHeader}>
                        ...
                        >
                            <div style={{...styles.tooltip, ...(isNameHovered ? styles.tooltipVisible : {})}}>
                                Ver Perfil
                            </div>
                            <h3 style={{...styles.patientName, ...(isNameHovered ? styles.patientNameHover : {})}}>
                                {patient.displayName || 'Paciente sem nome'}
                            </h3>
                            <p style={styles.patientDetails}>{patient.email}</p>
                        </div>
