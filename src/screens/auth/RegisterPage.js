...
const RegisterPage = () => {
  ...
  const { role } = useParams(); // esperado: 'patient'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate(`/login/${role || 'patient'}`), 1000);
    } catch (err) {
      console.error('Erro no cadastro (paciente):', err);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.loginBox}>
        {/* Botões de voltar (topo, dentro da caixa) */}
        <div style={styles.topRow}>
          <button style={styles.backBtn} onClick={() => navigate(`/login/${role || 'patient'}`)}>
            ← Voltar para Login
          </button>
